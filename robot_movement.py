...
def stream_save(camera_idj, ids,pig_id, i,j):
    
    
    if int(pig_id) == 0:
        path ="./Data/" +"STALL_"+ ids + "/"#+imgname
        filename = ids
    else:
        path ="./Data/" +"PIG"+ pig_id + "/"#+imgname
...
config_2 = rs.config()
config_2.enable_device(camera_id[1])
    ...
    if t1.minute % 5== 0:
        ...
        for i in range(r):
            ...
            for j in range(c):
                ...
                #autopilot=False
                if monitoring[j,i]==1:
                    stream_save(camera_id[j], ids[j,i],stall_sow_id[j,i], i,j)
                    '''
                    try:
                            #print(camera_id[j])
                        stream_save(camera_id[j], ids[j,i],stall_sow_id[j,i], i,j)

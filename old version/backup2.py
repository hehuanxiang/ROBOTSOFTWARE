...
#print(stall_sow_id)
def long_operation_thread(running_rob, gui_queue):
    running_rob = True
    leav_dock = True
    stream_one = False
    autopilot = False
        ...
        if leave_docking == True:
            ...
            if f_count <50:
                ...
                try:
                    ...
                    thread1=saveDataThread((col+2*row), aligned_frames,intr, "/home/pi/Desktop/ROBOTSOFTWARE/Capture", stall_id)
                    thread1.start()
                        #saveData(aligned_frames,intr, "/home/pi/Desktop/ROBOTSOFTWARE/Capture", stall_id)
                    e=0
                except:
                ...
                if row>=r:
                    ...
                    move_robot_forward = False
                    stream_one=False
                print("return to dock: ",return_to_dock)
                print("move camera forward: ",move_robot_forward)
                print("select camera: ", select_camera)
                print("stream_one: ", stream_one)
                print("leave_docking: ",leave_docking)
        if(datetime.now().minute ==0 and datetime.now().second<0.5):
            print(datetime.now())
            print("return to dock: ",return_to_dock)
            print("move camera forward: ",move_robot_forward)
            print("select camera: ", select_camera)
...
def run_main_gui(run_window):
    ...
    while run_window:
        ...
        if event == sg.WIN_CLOSED or event == "Exit":
            ...
            #leave_docking = True       #######################
            try:
                threading.Thread(target = long_operation_thread, args = (1,gui_queue), daemon=True).start()
